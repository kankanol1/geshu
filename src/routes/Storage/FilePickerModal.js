...
export default class FilePickerModal extends PureComponent {
  ...
  render() {
    const { visible, title, pickerHeight, okText,
      cancelText, onOk, onCancel, hideCancel,
      enableUpload, enableMkdir, smallSize, allowSelectFolder,
    } = this.props;
    const cancelBtn = <Button key="back" onClick={() => onCancel()}>{cancelText}</Button>;
    const okBtn = (
