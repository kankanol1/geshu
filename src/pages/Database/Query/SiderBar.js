...
const columns = [
  {
    ...
    width: 80,
    render: v => (
      <span style={{ fontWeight: '500', display: 'inline-block', width: '90px' }}>{v}</span>
    ),
  },
  {
    title: '类型',
        ...
        <List
          ...
          renderItem={item => (
            <List.Item
              ...
              className={item === this.state.selectedItem ? styles.selectedItem : null}
            >
      ...
      >
        ...
        <TabPane tab="元信息" key="meta">
          <div style={{ padding: '20px' }}>
            描述：
            {selectedItem.description === undefined ? '暂无' : selectedItem.description}
          </div>
        </TabPane>
      </Tabs>
    );
