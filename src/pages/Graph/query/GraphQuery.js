...
class GraphQuery extends React.PureComponent {
  ...
  handleVariableConfirmOk = variable => {
    ...
    this.props.dispatch({
      ...
      payload: {
        ...
        querySaveName: this.props.querySaveName,
      }, // eslint-disable-line
    });
    this.props.dispatch({
      ...
      if (searchArr != null) {
        searchArr.map(item => {
          ...
          for (let i = 0; i < variableListAll.length; i++) {
            ...
            if ('${' + variableListAll[i].variableName + '}' === item) {
              // eslint-disable-line
              isExist = true;
              break;
            }
  ...
  render() {
    const editorName =
      this.state.editorName === '' ? 'Gremlin编辑器' : `正在编辑：${this.state.editorName}`;
    return (
      <Layout style={{ padding: '0', height: '100%' }} theme="light">
        ...
        >
          <QueryTable
            onOpen={(query, id, name, variableList, code) => {
              this.setState({
                show: false,
                editorName: name,
                variableList,
        ...
        <div style={{ width: '100%', height: 200, background: 'white', position: 'relative' }}>
          <PullUpFrame tagClick={this.tagClick} className={styles.pullBox}>
            <div
              className={this.state.variableRange ? styles.queryVariable : styles.queryVariableHide}
            >
              <QueryVariable
            ...
            >
              <div className={styles.variableRangeBox}>
                <i
                  className={
                    this.state.variableRange ? styles.variableRange : styles.variableRangeHide
                  }
          ...
          >
            ...
            >
              <Tabs.TabPane tab="关系图" disabled={!this.props.showGraph} key="2">
                <div style={{ marginTop: '-13px' }}>
                  ...
                  <div
                    id="container"
                    style={{
                      height: this.state.pullBoxRange
              ...
              <Tabs.TabPane tab="JSON" key="1">
                <div>
                  <Highlight className="json">{this.props.responseJson}</Highlight>
                </div>
              </Tabs.TabPane>
            </Tabs>
          </div>
