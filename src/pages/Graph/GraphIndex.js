...
class GraphIndex extends Component {
  ...
  getJumpUrl = item => {
    ...
    if (name === 'mapper' && (item.status === 'NEW' || item.status === 'SCHEMA_CREATED')) {
      return `/graph/${name}/not_create/${item.id}`;
    }
    if (name === 'explore' || name === 'query') {
      if (item.status === 'NEW' || item.status === 'SCHEMA_CREATED') {
        return `/graph/${name}/not_create/${item.id}`;
      }
      if (item.status === 'SCHEMA_EXECUTED') {
        return `/graph/${name}/not_data/${item.id}`;
      }
    }
    return `/graph/${name}/detail/${item.id}`;
  };
  ...
  render() {
    ...
    return (
      <Layout className={styles.contentLayout} theme="light">
        <OpenProjectForm {...parentMethods} modalOpenVisible={modalOpenVisible} />
        <CreateGraphForm
        ...
        <Card title="项目列表" className={styles.firstCard}>
          {loading ? (
            ...
            recentGraph.data.map(item => (
              <p key={item.id}>
                <Link to={this.getJumpUrl(item)} className={styles.clickableItem}>
                  <Icon type="file" /> {item.name} (最后编辑于{' '}
