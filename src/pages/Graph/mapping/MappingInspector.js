...
function getEdgeMappingConfig(type) {
  ...
  if (currentInspectedObject && currentInspectedObject.data) {
    if (type === 'start') {
      const startPKResult = graphUtils.getNodePKProps(
        myDiagram,
      ...
      const endPKResult = graphUtils.getNodePKProps(
        myDiagram,
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            ...
            >
              {attrList.map(attr => (
                <Select.Option value={attr} key={attr}>
                  {attr}
                </Select.Option>
              ))}
            </Select>
    ...
    const linkAttrConfig = (
      <div className={styles.attrItem} style={{ width: '95%', marginTop: '10px' }}>
        <div className={`${styles.attrBox} ${styles.markdown}`}>
          ...
          <Row
            style={{ borderBottom: '1px dashed #ebedf0', paddingBottom: '10px' }}
            type="flex"
          ...
          >
                  ...
                  <Row>
                    <Col span={12}>
                      <Select
                        ...
                        optionFilterProp="children"
                        value={
                          this.state.startColumn ? this.state.startColumn.split(',')[index] : ''
                        }
                        onChange={value => {
                    ...
                    <Col span={12}>
                      <Tag style={{ padding: '0 10px' }}>{o}</Tag>
                    </Col>
                  </Row>
                );
              })}
            ...
            <Col span={6} style={{ textAlign: 'left' }}>
              <strong>{startPKResult.vertex}</strong>
            </Col>
          </Row>
          <Row style={{ paddingTop: '10px' }} type="flex" justify="space-around" align="middle">
            ...
            <Col span={6} style={{ textAlign: 'left' }}>
              <strong>{endPKResult.vertex}</strong>
            </Col>
          </Row>
        </div>
      </div>
