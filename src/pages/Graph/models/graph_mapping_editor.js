...
function inverseMap(map) {
  ...
  for (const key in map) {
    if (key && map[key] && key !== '__check' && key !== 'check') {
...
function getPath(str) {
  const txt = str.replace(
    /(([a-zA-z0-9]+:\/\/)|(\/)){0,1}((([0-9]{1,3}\.){3}[0-9]{1,3})|(([0-9a-z]+\.)+[0-9a-z]+))(:[0-9]{2,4}){0,1}/i,
    ''
  );
  return txt;
}
...
export default {
  ...
  reducers: {
    init(state, { payload }) {
      ...
      const diagram = graphUtil.initMappingDiagram(
        graphContainer,
  ...
  effects: {
    ...
    *getDataSourceColumns({ payload }, { call, put, select }) {
      ...
      const { datasourceId2Columns } = yield select(state => state.graph_mapping_editor);
    ...
    *saveMapping({ payload }, { call, put, select }) {
      ...
      const { id } = yield select(state => state.graph_mapping_editor);
      ...
      const response = yield call(saveGraph, {
        frontendMappingJson,
        backendMappingJson,
        id,
      });
      if (response) {
        if (!payload) {
          ...
          if (execution.success) {
            yield put(routerRedux.push('/graph/jobs'));
          } else {
