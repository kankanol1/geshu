...
export default {
  ...
  effects: {
    ...
    *searchGraph({ payload }, { call, put, select }) {
      const { id, type2Label2Attrs, type2Attrs, tableName } = yield select(
        state => state.graph_explore
      );
      ...
      if (searchValue.length <= 1 && type === 'node') {
        ...
        searchValue.map(item => {
          if (item.attr && item.attrData) {
            basicSearch += `.has('${item.attr}',${
              Number(item.attrData) ? item.attrData : `'${item.attrData}'`
            })`;
          }
          if (item.type) {
      ...
      const response = yield call(queryGremlinServer, {
        code:
          !tableName || tableName === 'g'
            ? code
            : `g=ConfiguredGraphFactory.open('${tableName}').traversal();${code}`,
        id,
      });
      if (response.status.code > 200) {
        message.error(`错误：${response.status.message}`);
      }
      yield put({
        type: 'setGraph',
        payload: response,
    ...
    *searchRouteGraph({ payload }, { call, put, select }) {
      ...
      if (searchValue.routeRange === 1) {
        basicSearch = `es=g.V().has('${searchValue.beginRouteType}', '${
          searchValue.beginRouteAttr
        }','${searchValue.beginRouteData}').repeat(bothE().otherV().simplePath()).until(has('${
          searchValue.endRouteType
        }', '${searchValue.endRouteAttr}','${searchValue.endRouteData}').or().loops().is(${
          searchValue.routeNum
        })).has('${searchValue.endRouteType}', '${searchValue.endRouteAttr}','${
          searchValue.endRouteData
        }').path().by(id).limit(1).collectMany{it.grep(org.janusgraph.graphdb.relations.RelationIdentifier)};sg=g.E(es).subgraph(\'sg\').cap(\'sg\').next();sgt=sg.traversal();[sg.traversal().V().toList(),sg.traversal().E().toList()]`;
        ...
        basicSearch = `es=g.V().has('${searchValue.beginRouteType}', '${
          ...
          searchValue.endRouteData
        }').path().by(id).limit(1).collectMany{it.grep(org.janusgraph.graphdb.relations.RelationIdentifier)};sg=g.E(es).subgraph(\'sg\').cap(\'sg\').next();sgt=sg.traversal();jump=sgt.E().count().next();es1=g.V().has('${
          searchValue.beginRouteType
        }', '${searchValue.beginRouteAttr}','${
          searchValue.beginRouteData
        }').repeat(bothE().otherV().simplePath()).until(has('${searchValue.endRouteType}', '${
          searchValue.endRouteAttr
        }','${searchValue.endRouteData}').or().loops().is(jump)).has('${
          searchValue.endRouteType
        }', '${searchValue.endRouteAttr}','${
          searchValue.endRouteData
        }').path().by(id).collectMany{it.grep(org.janusgraph.graphdb.relations.RelationIdentifier)};sg1=g.E(es1).subgraph(\'sg1\').cap(\'sg1\').next();sgt1=sg1.traversal();[sgt1.V().toList(),sgt1.E().toList()]`;
        ...
        basicSearch = `es=g.V().has('${searchValue.beginRouteType}', '${
          ...
          searchValue.endRouteData
        }').path().by(id).collectMany{it.grep(org.janusgraph.graphdb.relations.RelationIdentifier)};sg=g.E(es).subgraph(\'sg\').cap(\'sg\').next();sgt=sg.traversal();[sg.traversal().V().toList(),sg.traversal().E().toList()]`;
      }
