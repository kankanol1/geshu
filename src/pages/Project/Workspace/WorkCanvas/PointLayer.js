...
class PointLayer extends React.Component {
  ...
  handleDragStop(e) {
    ...
    Object.entries(canvas.componentSocketPositionCache).forEach(([id, positionDict]) => {
      Object.entries(positionDict).forEach(([pid, { x, y }]) => {
        if (
          Math.abs(x - draggingTarget.x) <= overlapRange &&
      ...
      if (draggingMetaType === 'input') {
        ...
        if (candidate.component.id === draggingComponent) {
          ...
          if (model.connections.filter(conn => conn.to === draggingPoint).length === 0) {
            ...
            const newOp = new ConnectionAdd(
              model,
              new Connection(candidate.component.id, candidate.pointId, draggingPoint)
            );
            canvas.apply(newOp);
          } else {
    ...
    if (overlappedInputs.length !== 0) {
      if (draggingMetaType === 'output') {
        ...
        if (candidate.component.id === draggingComponent) {
          ...
          if (
            candidate.component.connections.filter(conn => conn.to === candidate.pointId).length ===
            0
          ) {
            const newOp = new ConnectionAdd(
              candidate.component,
              new Connection(draggingComponent, draggingPoint, candidate.pointId)
            );
            canvas.apply(newOp);
          } else {
            message.warn('输入点已连接，一个输入点仅能连出一条线');
  ...
  handleDrag(e, draggableData, point, originPos) {
    ...
    this.props.dispatch({
      ...
      draggingTarget: {
        x: (draggingTarget.x == null ? originx : draggingTarget.x) + deltaX * scaleParam,
        y: (draggingTarget.y == null ? originy : draggingTarget.y) + deltaY * scaleParam,
      },
