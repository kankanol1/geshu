...
class ComponentSettingsForm extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  render() {
    const {
      ...
      componentSettings,
      display,
    } = this.props.work_component_settings;
    ...
    return (
      <Fragment>
        ...
        <Affix
          offsetBottom={10}
          style={{ height: '46px', textAlign: 'center', background: '#fafafa' }}
        >
          <Button
            style={{ margin: '10px 50px' }}
            type="danger"
            disabled={jsonSchema === undefined}
          ...
          <Button style={{ margin: '10px 50px' }} onClick={e => this.submitForm(e)}>
            {' '}
            <Icon type="save" />
            应用{' '}
          </Button>
          <Button
            style={{ margin: '10px 50px' }}
            type="primary"
            onClick={e => this.submitFormAndClose(e)}
          >
            {' '}
            <Icon type="save" />
            保存并关闭{' '}
