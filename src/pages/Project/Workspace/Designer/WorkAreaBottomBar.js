...
@connect(({ workcanvas, loading }) => ({
  tips: workcanvas.tips,
  loading,
}))
class WorkAreaBottomBar extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      fullView: false,
  ...
  renderMessage = (message, k = null) => {
    return message ? (
      <div key={k}>
        {moment(message.time).format('HH:mm:ss.SSS')}: {message.message}
      </div>
    ) : null;
  };
...
export default WorkAreaBottomBar;
