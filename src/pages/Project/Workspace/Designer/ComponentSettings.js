...
class ComponentSettings extends React.PureComponent {
  ...
  renderSettings = displaySettings => {
    return (
      <div>
        {Object.entries(displaySettings.properties).map(([k, item]) => {
          return <BasicParamInput key={k} title={item.description} name={k} type={item.title} />;
        })}
      </div>
    );
  };
        ...
        title={
          loading
            ...
            : displaySettings === undefined
              ? '未获取到配置'
              : translateName(displaySettings.title)
        }
        visible
        footer={null}
        // onOk={() => this.handleOk()}
      ...
      >
        ...
        <div style={{ height: '500px' }}>
          ...
          Object.entries(displaySettings.properties).length === 0 ? (
            <div style={{ paddingTop: '20%', textAlign: 'center' }}>无可配置项</div>
          ) : (
