...
@connect(({ work_component_list, loading }) => ({
  work_component_list,
  loading: loading.models.work_component_list,
}))
class TopComponentList extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      preview: null,
  ...
  handlePreviewChange(preview) {
    if (preview === null) {
      ...
      const displayPreview = React.cloneElement(preview, {
        style: {
          ...preview.props.style,
          ...{
      ...
      <div className={styles.topTabWrapper}>
        {
          <Tabs
            type="card"
            style={{ height: this.state.showTop ? undefined : '0px' }}
            className={styles.topTabs}
          >
            {groups.map(group => {
              return group.components.length === 0 ? null : (
...
export default TopComponentList;
