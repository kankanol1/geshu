...
export default class ConfigurationTable extends React.Component {
  ...
  renderCell = (c, item, index) => {
    const { cellWrapper } = this.props;
    let Comp = c.render(item[c.name], item, this.onChange(item, index, c.name), index);
    if (cellWrapper) {
      Comp = cellWrapper(c, item, index, Comp);
    }
    return Comp;
  };
